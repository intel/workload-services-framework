...
if [[ "${TESTCASE}" =~ "ebpf" ]]; then
    EBPF=ebpf
    if [[ "${TESTCASE}" =~ "1n" ]]; then
        CLIENT_HOST_NETWORK=false
    fi
fi

if [[ "${TESTCASE}" =~ "avx512" ]]; then
    AVX512_PATCH=avx512
fi
...
-DQAT_RESOURCE_TYPE=$QAT_RESOURCE_TYPE
-DCY_NUM=$CY_NUM
-DDLB_NUM=$DLB_NUM
