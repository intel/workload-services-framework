...
def parse_args():
    ...
    parser.add_argument('--compile_openvino', action='store_true', default=False, help='compile with openvino backend')
...
def main():
    ...
    if args.compile_openvino:
        print("Running PyTOrch+OpenVINO ...")
        import openvino.torch
        if args.precision == "bf16":
            opt = {"device" : "CPU", "config" : {"INFERENCE_PRECISION_HINT": "bf16"}}
        else:
            opt = {"device" : "CPU", "config" : {"INFERENCE_PRECISION_HINT": "f32"}}
        pipe.text_encoder = torch.compile(pipe.text_encoder.eval(), backend='openvino',options=opt)
        pipe.unet = torch.compile(pipe.unet.eval(), backend='openvino',options=opt)
        pipe.vae = torch.compile(pipe.vae.eval(), backend='openvino',options=opt)
