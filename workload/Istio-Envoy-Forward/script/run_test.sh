...
# Server setting
SERVER_DOMAIN_NAME=${SERVER_DOMAIN_NAME:-nighthawk-server.istio-workload-nighthawk-server.svc.cluster.local}
...
CLIENT_CORE_NUM=${CLIENT_CORE_NUM:-8}
CLIENT_CONNECTIONS=${CLIENT_CONNECTIONS:-1000}
CLIENT_CONCURRENCY=${CLIENT_CONCURRENCY:-40}
CLIENT_RPS=${CLIENT_RPS:-200}
CLIENT_RPS_MAX=${CLIENT_RPS_MAX:-2000}
CLIENT_RPS_STEP=${CLIENT_RPS_STEP:-200}
...
# wait nh-server service ready
until nc -z -w5 "$SERVER_DOMAIN_NAME" "$SERVER_PORT"; do
    echo "Waiting nh-server service ready ..."
    sleep 1s
done
...
nighthawk_test() {
	...
	echo "	- Server Domain Name: $SERVER_DOMAIN_NAME"
	echo "	- Port: $PORT"
	echo "	- RPS: $CLIENT_RPS"
	echo "	- Duration: $1 sec"
	...
	if [[ "$PROTOCOL" == "http1" ]]; then
		...
		echo
		echo "nighthawk_client -p "$PROTOCOL" --connections "$CLIENT_CONNECTIONS" --request-body-size "${CLIENT_REQUEST_BODY_SIZE}" --concurrency "$CLIENT_CONCURRENCY" --rps "$CLIENT_RPS"  --duration "$1" "$SERVER_DOMAIN_NAME":"$PORT" > "$CLIENT_RPS".log"
		nighthawk_client -p "$PROTOCOL" --connections "$CLIENT_CONNECTIONS" --request-body-size "${CLIENT_REQUEST_BODY_SIZE}" --concurrency "$CLIENT_CONCURRENCY" --rps "$CLIENT_RPS"  --duration "$1" "$SERVER_DOMAIN_NAME":"$PORT" > "$CLIENT_RPS".log
		stat=$?
...
echo -e "\\n"
echo "Start Input RPS Scaling. Client request body size is ${CLIENT_REQUEST_BODY_SIZE}B, Server response body size is ${SERVER_RESPONSE_BODY_SIZE}B:"
