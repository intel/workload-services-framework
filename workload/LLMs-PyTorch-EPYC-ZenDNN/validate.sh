...
OPTION=${1:-inference_throughput_bfloat16_pkm}
PLATFORM=${PLATFORM:-SPR}
WORKLOAD=${WORKLOAD:-GPTJ-PyTorch-EPYC-zenDNN}
...
if [[ "${MODEL_NAME}" == *"gpt-j"* ]]; then
    ...
    MODEL_PATH=${MODEL_PATH:-"/opt/dataset/llama2/7b"}
...
JOB_FILTER="job-name=gptj-pytorch-epyc-zendnn"
