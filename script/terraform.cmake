...
function(add_backend_testcase type component name)
    foreach(sut1 ${sut_reqs})
        if(sut1)
            ...
            add_testcase_1("${sut2}_${name}" "BACKEND=${BACKEND} TERRAFORM_SUT=${sut1}" ${ARGN})
        endif()
    endforeach()
endfunction()
...
if (NOT TERRAFORM_OPTIONS)
    set(TERRAFORM_OPTIONS "--docker --sutinfo --intel_publish")
endif()

if (NOT "${TERRAFORM_OPTIONS} " MATCHES "--(native|compose|kubernetes|docker) ")
    set(TERRAFORM_OPTIONS "${TERRAFORM_OPTIONS} --docker")
endif()
...
if (TERRAFORM_SUT STREQUAL "")
    set(TERRAFORM_SUT "${sut_all}" CACHE STRING "" FORCE)
elseif (NOT DEFINED TERRAFORM_SUT)
    set(TERRAFORM_SUT "static" CACHE STRING "" FORCE)
...
string(REPLACE " " ";" suts "${TERRAFORM_SUT}")
set(TERRAFORM_SUT "" CACHE STRING "" FORCE)
foreach(sut1 ${suts})
    if(sut1 MATCHES ":")
        string(REGEX REPLACE "^.*:" "" sut1_ct "${sut1}")
        math(EXPR sut1_ct "${sut1_ct} - 1")
        string(REGEX REPLACE ":.*$" "" sut1_bn "${sut1}")
        foreach(sut1_ct1 RANGE ${sut1_ct})
            set(TERRAFORM_SUT "${TERRAFORM_SUT}${sut1_bn}${sut1_ct1} " CACHE STRING "" FORCE)
        endforeach()
    else()
        set(TERRAFORM_SUT "${TERRAFORM_SUT}${sut1} " CACHE STRING "" FORCE)
    endif()
endforeach()
string(REPLACE " " ";" suts "${TERRAFORM_SUT}")
foreach(sut1 ${suts})
    if(NOT " ${sut_all} " MATCHES " ${sut1} ")
        message(FATAL_ERROR "Failed to locate terraform config: ${sut1}")
    endif()
endforeach()

if(TERRAFORM_REGISTRY AND (NOT (TERRAFORM_REGISTRY MATCHES ".*/")))
    set(TERRAFORM_REGISTRY "${TERRAFORM_REGISTRY}/")
endif()
if(TERRAFORM_RELEASE AND (NOT (TERRAFORM_RELEASE MATCHES ":.*")))
    set(TERRAFORM_RELEASE ":${TERRAFORM_RELEASE}")
endif()
