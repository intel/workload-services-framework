...
data "external" "ip" {
  ...
  program = [ "${path.module}/scripts/get-ip.sh", var.kvm_host.networks.0, libvirt_domain.default[each.key].network_interface.0.mac, "-p", var.kvm_host.port, "${var.kvm_host.user}@${var.kvm_host.host}" ]
}
...
resource "null_resource" "https" {
  ...
  provisioner "local-exec" {
    command = "ssh -fNL $LPORT:$IP:$RPORT -p $SSH_PORT $SSH_USER@$SSH_HOST"
    environment = {
      LPORT     = each.value
